...
DATA_ROOT = Path(os.environ.get("LCQAOA_DATA_ROOT", PROJECT / "raw_data"))
...
def make_figure() -> None:
    ...
    # D. Same-device paired confidence intervals; ties use open markers and a printed label.
    ax = axes[3]
    routes = [
    ...
    ax.set_title("D. Matched Crossover (device-specific)")
    ax.legend(frameon=False, loc="upper left", handlelength=1.5, ncol=1)

    # B. Active allocation follows B*2^k while runtime quickly saturates.
    ax = axes[1]
    ...
    ax.set_title(r"B. Microbatch Pareto ($n=16{,}384$)")
    ax.legend(frameon=False, loc="upper right", handlelength=1.5)

    # C. Normalize every policy to cache-all on the same graph instance.
    ax = axes[2]
    ...
    styles = {
        ...
        "budgeted_0.25": ("budget 25%", ORANGE, "v"),
        "budgeted_0.50": ("budget 50%", ORANGE, "<"),
        "budgeted_1.00": ("budget 100%", ORANGE, ">"),
    }
    annotation_offsets = {
        "cache_all": (8, -18),
        "fixed_interval": (-34, -5),
    ...
    ax.set_xlabel("active allocated / cache-all")
    ax.set_ylabel("time / cache-all")
    ax.text(
        0.03,
        0.96,
        "215 feasible / 270 requests\n55 rejected pre-allocation",
    ...
    ax.set_title("C. Checkpoint Schedules")
